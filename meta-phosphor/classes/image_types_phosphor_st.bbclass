...
# Image composition
FLASH_KERNEL_IMAGE ?= "kernel/fitImage-${INITRAMFS_IMAGE}-${MACHINE}-${MACHINE}"
FLASH_KERNEL_IMAGE:df-obmc-ubi-fs ?= "kernel/fitImage-${MACHINE}.bin"

# Default value, to modify in machine config
FLASH_METADATA_IMAGE ?= "metadata.bin"
FLASH_TF_A_IMAGE ?= "tf-a-stm32mp257f-ev1-optee-nor.stm32"
FLASH_FIP_IMAGE ?= "fip-stm32mp257f-ev1-optee-nor.bin"
...
FLASH_TF_A_1_OFFSET ?= "0"
FLASH_TF_A_2_OFFSET ?= "256"
FLASH_METADATA_1_OFFSET ?= "512"
FLASH_METADATA_2_OFFSET ?= "768"
FLASH_FIP_OFFSET ?= "1024"
FLASH_UBOOT_ENV_OFFSET = "5120"
FLASH_MANIFEST_OFFSET ?= "5632"
FLASH_KERNEL_OFFSET ?= "5632"
FLASH_ROFS_OFFSET ?= "18240"
FLASH_RWFS_OFFSET ?= "44800"
...
do_make_ubi() {
    ...
    dd bs=1k conv=notrunc seek=${FLASH_TF_A_1_OFFSET} \
        if=${DEPLOY_DIR_IMAGE}/u-boot/u-boot-${UBOOT_SUFFIX} \
...
do_generate_image_uboot_file() {
    image_dst="$1"
    uboot_offset=${FLASH_TF_A_1_OFFSET}

    if [ ! -z ${SPL_BINARY} ]; then
        dd bs=1k conv=notrunc seek=${FLASH_TF_A_1_OFFSET} \
            if=${DEPLOY_DIR_IMAGE}/u-boot/u-boot-spl-${UBOOT_SUFFIX} \
            of=${image_dst}
...
python do_generate_static() {
    ...
    # _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE', True), 'u-boot/',
    #                            'u-boot-%s' % d.getVar('UBOOT_SUFFIX',True)),
    #               uboot_offset,
    #               int(d.getVar('FLASH_UBOOT_ENV_OFFSET', True)))

    # _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE', True), 'kernel/',
    ...
    # Adaptation to STM32MP2 BSP (including TF-A, Metadata, FIP ...)

    _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE', True), 'arm-trusted-firmware/',
                               d.getVar('FLASH_TF_A_IMAGE',True)),
                  int(d.getVar('FLASH_TF_A_1_OFFSET', True)),
                  int(d.getVar('FLASH_TF_A_2_OFFSET', True)))

    _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE', True), 'arm-trusted-firmware/',
                               d.getVar('FLASH_TF_A_IMAGE',True)),
                  int(d.getVar('FLASH_TF_A_2_OFFSET', True)),
                  int(d.getVar('FLASH_METADATA_1_OFFSET', True)))

    _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE', True), 'arm-trusted-firmware/',
                               d.getVar('FLASH_METADATA_IMAGE',True)),
                  int(d.getVar('FLASH_METADATA_1_OFFSET', True)),
                  int(d.getVar('FLASH_METADATA_2_OFFSET', True)))

    _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE', True), 'arm-trusted-firmware/',
                               d.getVar('FLASH_METADATA_IMAGE',True)),
                  int(d.getVar('FLASH_METADATA_2_OFFSET', True)),
                  int(d.getVar('FLASH_FIP_OFFSET', True)))

    _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE', True), 'fip/',
                               d.getVar('FLASH_FIP_IMAGE',True)),
                  int(d.getVar('FLASH_FIP_OFFSET', True)),
                  int(d.getVar('FLASH_UBOOT_ENV_OFFSET', True)))
...
python do_generate_static_norootfs() {
    ...
    uboot_offset = int(d.getVar('FLASH_TF_A_1_OFFSET', True))
